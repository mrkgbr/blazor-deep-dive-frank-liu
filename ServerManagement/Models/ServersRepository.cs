...
public static class ServersRepository
{
    ...
    {
        return _servers.Where(s => s.City != null && s.City.Equals(cityName, StringComparison.OrdinalIgnoreCase)).ToList();
    }
    ...
    {
        return _servers.Where(s => s.Name != null && s.Name.Contains(serverFilter, StringComparison.OrdinalIgnoreCase)).ToList();
    }
}